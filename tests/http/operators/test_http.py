...
def test_http_response_check_does_not_run_async():
    ...
    with mock.patch(
        "astronomer.operators.http.operators.http.HttpSensorAsync.defer"
    ...
    with mock.patch("astronomer.operators.http.operators.http.HttpSensorAsync.defer") as mock_defer:
        operator.execute({})
