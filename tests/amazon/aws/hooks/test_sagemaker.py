...
class TestSagemakerHookAsync:
    ...
    # @pytest.mark.asyncio
    # @mock.patch("astronomer.providers.amazon.aws.hooks.sagemaker.SageMakerHookAsync.get_multi_stream")
    # @mock.patch("astronomer.providers.amazon.aws.hooks.sagemaker.SageMakerHookAsync.describe_training_job_async")
    # @mock.patch("astronomer.providers.amazon.aws.hooks.aws_logs.AwsLogsHookAsync.describe_log_streams_async")
    # @mock.patch("astronomer.providers.amazon.aws.hooks.sagemaker.SageMakerHookAsync.get_client_async")
    # async def test_sagemaker_training_job(self, mock_client,
    #                                       mock_describe_log_stream, mock_training_job, mock_multi_stream):
    #     """Test sagemaker training with log async hook function to describe the training job using Aiobotocore lib"""
    #     mock_describe_log_stream.return_value = {
    #             'logStreams': [
    #                 {
    #                     'logStreamName': 'test',
    #                     'creationTime': 123,
    ...
    #             'nextToken': 'test'
    #         }
    #     mock_training_job.return_value = {"TrainingJobStatus": "InProgress"}
    #     hook = SageMakerHookAsync(aws_conn_id="test_conn_id")
    #     result = await hook.describe_training_job_with_log(job_name="test_job")
    #     assert result == {"test": "response"}
