...
def test_databricks_trigger_serialization():
    ...
    assert classpath == "astronomer.operators.databricks.triggers.databricks.DatabricksTrigger"
    assert kwargs == {
        "conn_id": CONN_ID,
        "task_id": TASK_ID,
        "run_id": RUN_ID,
...
@pytest.mark.asyncio
@mock.patch("astronomer.operators.databricks.hooks.databricks.DatabricksHookAsync.get_run_state_async")
