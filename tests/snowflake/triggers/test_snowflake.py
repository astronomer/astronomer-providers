...
from astronomer.operators.snowflake.triggers.snowflake_trigger import SnowflakeTrigger
...
def test_snowflake_trigger_serialization():
    ...
    assert classpath == "astronomer.operators.snowflake.triggers.snowflake_trigger.SnowflakeTrigger"
    assert kwargs == {
        "task_id": TASK_ID,
        "polling_period_seconds": 1.0,
        "query_ids": [],
...
@pytest.mark.asyncio
@mock.patch("astronomer.operators.snowflake.hooks.snowflake.SnowflakeHookAsync.get_query_status")
...
@pytest.mark.asyncio
@mock.patch("astronomer.operators.snowflake.hooks.snowflake.SnowflakeHookAsync.get_conn")
@pytest.mark.parametrize(
    "query_ids",
    [
        (["uuid", "uuid"]),
