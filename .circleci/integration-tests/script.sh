...
if [[ ${DEPLOYMENT_INSTANCE} == "staging" ]]; then
  ...
  docker build --target "${DEPLOYMENT_INSTANCE}" -t "${IMAGE_NAME}" -f "${SCRIPT_PATH}"/Dockerfile.gen_1 "${SCRIPT_PATH}"
  ...
  docker build --target "${DEPLOYMENT_INSTANCE}" -t "${IMAGE_NAME}" -f "${SCRIPT_PATH}"/Dockerfile.astro_cloud "${SCRIPT_PATH}"
